...
/**
 * Kept as the single entry point every sign-in path already calls, but the work
 * now happens in lib/sessions.js: a SHORT access token plus a stored, revocable
 * refresh token. The old behaviour — one stateless 30-day token that nothing
 * could withdraw — is what the sessions table exists to end.
 */
async function issueSession(user, rememberMe = true, context = {}) {
    const sessions = require('./sessions');
    const s = await sessions.createSession(user, { rememberMe, ...context });
    if (s.success) return s;
    return legacyIssueSession(user, rememberMe);
}

// Fallback for a box where 004-sessions.sql has not been applied yet: the old
// single long-lived token. Logged loudly, because it is a silent downgrade of
// everything the split buys.
async function legacyIssueSession(user, rememberMe = true) {
    console.error('[local-account] session store unavailable — falling back to a single long-lived token; run npm run migrate');
    return legacyIssueSessionImpl(user, rememberMe);
}

async function legacyIssueSessionImpl(user, rememberMe = true) {
