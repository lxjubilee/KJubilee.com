...
async function requestReset(rawEmail, requestedIp) {
    ...
    const sent = await email.sendPasswordResetEmail({
        to: addr, token, minutes: TTL_MINUTES,
        // Only known when there is an account here; the greeting handles absence.
        firstName: local.exists ? local.user.first_name : null,
    });
    if (!sent.success) {
