...
async function requestReset(rawEmail, requestedIp) {
    ...
    if (!local.exists) return quiet;               // no account — say nothing
    if (local.user.is_locked || local.user.is_active === false) return quiet;
...
async function completeReset(token, newPassword) {
    ...
    if (!local.success || !local.exists) {
        return { success: false, error: 'That reset link is no longer valid. Please request a new one.' };
    }

    const usesJubileeId = Boolean(local.user.jubilee_id) || !local.user.has_local_password;
