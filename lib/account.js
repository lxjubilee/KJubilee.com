 ...
 *
 * Asked before a password change and before a deletion — the two acts that a
 * stolen fifteen-minute access token would otherwise be enough to commit.
 *
 ...
 */
async function changePassword(userId, { currentPassword, newPassword, rememberMe = true, userAgent, ip } = {}) {
    const u = await loadAccount(userId);
    if (!u) return { success: false, status: 401, error: 'Not signed in.' };
    ...
    const proof = await verifyPassword(u, currentPassword);
