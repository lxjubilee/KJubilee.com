 ...
 *
 * ── What stands between an account and this ──
 * The live session, and the typed word. Not a password: it was a second copy of
 * the proof the session already carried, and the person most likely to be
 * stopped by it was the owner who could not remember it — who then cannot leave
 * a site they have decided to leave. The typed DELETE is the lock that actually
 * matters here, because it catches the misfire this route really has to fear:
 * the owner who did not mean it. A password is muscle memory and a browser will
 * fill it in; typing DELETE cannot happen by accident.
 *
 * The route checks the word before calling this, and enforces the rate limit.
 */
